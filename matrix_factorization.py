        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                if control == 0:
                    print("******************************************"*4)
                    print(sub_x)
                    print(sub_y,type(sub_y),len(sub_y))
                pred, u_emb, v_emb = self.forward(sub_x, True)
                ...
                if control == 0:
                    print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
                    print(pred)
                    print(f"xent_loss: {xent_loss} type:{type(xent_loss)}")
                    print(xent_loss.detach())
                    print(f"epoch_loss: valuse:{epoch_loss}, type{type(epoch_loss)}")
...
class MF_IPS(nn.Module):
    ...
    def fit(self, x, y,y_logis, y_ips=None,
        ...
        # one_over_zl = torch.tensor(1/get_logis_propensity())
