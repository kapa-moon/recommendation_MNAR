...
def rating_mat_to_sample(mat):
    row, col = np.nonzero(mat)# row represent the indice of row that have number>0 mat[row, col] > 0
    ...
    print(f"this is y%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n{y}",)
    print(len(y))
    # one line, dimension is 1 and 6960 numbers > 0
    print(y)
    print(len(row),len(col))
    x = np.concatenate([row.reshape(-1,1), col.reshape(-1,1)], axis=1)
    print(x)
    # in this line, the x is a matrix and each row represent the coordinates of mat where number > 0
